...
var yPos = 0;
var shapePreview = [];
var xPreviewPos = 0; // coords from top left
var yPreviewPos = 0;
...
function setPreviewPiecePos() {
  shapePreview = structuredClone(currentShape)
  xPreviewPos = xPos
  yPreviewPos = (yPos)

  var blocksCanMoveDown = 0
  for (var i = 0; i < 20 - yPreviewPos; i++) {
    if (validMove(0, i)) {
      blocksCanMoveDown = i
    } else {
      break
  ...
  yPreviewPos += blocksCanMoveDown
  console.log(yPreviewPos)
}
...
function renderBoard() {
  for (var y = 0; y < ROWS; y++) {
    ...
    for (var x = 0; x < COLS; x++) {
      ...
      //draw preview piece
      var prevPiece = false;
      if (typeof shapePreview[y - yPreviewPos] != 'undefined') {
        var prevPiece = shapePreview[y - yPreviewPos][x - xPreviewPos]
      }
      if (prevPiece) { // if the current piece is on this coord then draw it
        if (prevPiece != 0) drawSquare(x, y, prevPiece, false, 1, false, true)
      }
      ...
      if (piece) { // if the current piece is on this coord then draw it
        if (piece != 0) drawSquare(x, y, piece, false, 1)
      } else { // draw whats on the board
...
function drawSquare(x, y, id, isQueue, gridTransparency = 1, isGrid = false, darker = false) { // coords from upper left corner
  ...
  if (darker) {
    thisCtx.globalCompositeOperation = "multiply"
    thisCtx.fillStyle = "rgb(200,200,200)";
    thisCtx.strokeRect(x * BlockPixelWidth + thisCtx.lineWidth / 2, y * BlockPixelHeight + thisCtx.lineWidth / 2, BlockPixelWidth - thisCtx.lineWidth, BlockPixelHeight - thisCtx.lineWidth);
    thisCtx.fillRect(x * BlockPixelWidth, y * BlockPixelHeight, BlockPixelWidth, BlockPixelHeight)
  }
  thisCtx.globalAlpha = 1
  thisCtx.globalCompositeOperation = "source-over"
}
