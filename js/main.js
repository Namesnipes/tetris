...
function rotate(dir){
	...
	if(!froze && currentShape && currentID != 3){
		if(dir == "CW"){
			...
			var rotatedShape = spinArrayCounterCockwise(JSON.parse(JSON.stringify(currentShape))) // clone array so it is not interfered with when looping thru
		}
